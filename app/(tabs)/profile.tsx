...
export default function ProfileScreen() {
  ...
  const handleClearTestActs = async () => {
    Alert.alert(
      'Clear Test Acts',
      'This will delete acts with titles like "Fuck you", "A", etc. Continue?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Clear',
          ...
          onPress: async () => {
            try {
              ...
              const testTitles = ['fuck you', 'a', 'test', 'asdf', 'aa', 'aaa'];

              const cleanedActs = allActs.filter(act => {
                const title = act.title.toLowerCase().trim();
                return !testTitles.includes(title) && title.length > 2;
              });

              await AsyncStorage.setItem('kindness_acts', JSON.stringify(cleanedActs));

              const removedCount = allActs.length - cleanedActs.length;

              Alert.alert(
                'Success!',
                `Removed ${removedCount} test act${removedCount !== 1 ? 's' : ''}.`
              );

              await recalculateStreak();
              await loadProfileData();
            } catch (error) {
              console.error('Error clearing test acts:', error);
              Alert.alert('Error', 'Failed to clear test acts');
            }
          }
        }
      ]
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      >
        <ThemedView style={styles.content}>
          ...
          <TouchableOpacity style={styles.clearTestButton} onPress={handleClearTestActs}>
            <ThemedText type="defaultSemiBold" style={styles.clearTestButtonText}>
              🧹 Clear Test Acts
            </ThemedText>
          </TouchableOpacity>
